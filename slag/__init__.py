...
def magic(path, para):
  ...
  This could probably be done through a Python-Markdown extension, but..."""
    ...
    if para.startswith("!file") or para.startswith("!code"):
        ...
        return Code(path=file, real_path=os.path.abspath(os.path.join(path, file)))
...
@click.option(
    "--pagesize", "-s", default=16, help="Number of posts to display per page"
)
@click.option(
    "--maxparagraphs", "-g", default=1, help="Number of preview paragraphs to display"
)
@click.option(
    "--hrefsuffix",
    "-x",
...
@click.option(
    ...
    "-d",
    default="%b %e, %I:%M%P",
    help="Format to pass to strftime() for dates",
)
...
def render_all(config, **kwargs):
    ...
    for path in paths:
        ...
        try:
            ...
            click.echo(
                click.style("    error: ", fg="red") + "unable to find git repository"
            )
            sys.exit(1)
        ...
        nav.append(Link(title=f"/{name}", href=f"{name}{href_suffix}"))
    ...
    def render_pages(posts, filename_fn, href_fn, title_fn):
        ...
        for i in range((len(posts) + (page_size - 1)) // page_size):
            pages.append(Link(title=f"{i + 1}", href=href_fn(i)))
    ...
    for name, posts in repos.items():
        ...
        render_pages(
            ...
            lambda i: f"{name}{href_suffix}"
            if i == 0
            else f"{name}/page-{i + 1}{href_suffix}",
